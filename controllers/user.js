...
function loginUser(req,res){
    ...
    User.findOne({email: email.toLowerCase()},(err,user)=>{
        if(err){
            ...
            if(!user){
                res.status(404).send({message: 'Error de login 1'});
            }else{
                bcrypt.compare(password, user.password,(err, check)=>{
                    ...
                    // devolver datos de usuario logueado
                        ...
                        res.status(404).send({message: 'Error de login2' });
                    }
                })
            }
        }
...
function uploadImage(req,res){
    ...
    if(req.files){
        ...
        if(file_ext.toLowerCase() == 'png' || file_ext.toLowerCase() == 'jpg' || file_ext.toLowerCase() == 'gif'){
            User.findByIdAndUpdate(userId, {image: file_name},(err, userUpdateIma)=>{
                if(err){
                    res.status(500).send({message: 'error al actulizar'});
                }else{
                    if(!userUpdateIma){
                        ...
                        res.status(200).send({user: userUpdateIma});
                    }
                }
